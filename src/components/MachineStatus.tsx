...
const MachineCard: React.FC<{
  ...
  const filteredSearchBlocks = React.useMemo(() => {
    if (!searchQuery || selectedBlockId) return [];
    const q = searchQuery.toLowerCase();
    return availableBlocks.filter(b => 
      b.jobNo.toLowerCase().includes(q) || 
      b.company.toLowerCase().includes(q) || 
      b.material.toLowerCase().includes(q)
    ).slice(0, 6);
  }, [availableBlocks, searchQuery, selectedBlockId]);
  ...
  const handleSelectBlock = (block: Block) => {
    ...
    setSearchQuery(`#${block.jobNo} | ${block.company}`);
                ...
                <div>
                  ...
                  <div className="relative">
                    <input 
                      ...
                      placeholder="Type Job No..."
                      value={searchQuery}
                      onChange={handleSearchChange}
                      onFocus={() => setShowResults(true)}
                      disabled={isGuest || isSubmitting}
                    />
                    {showResults && filteredSearchBlocks.length > 0 && (
                      <div className="absolute z-50 w-full mt-1 bg-white border border-[#d6d3d1] rounded-xl shadow-2xl overflow-hidden animate-in fade-in slide-in-from-top-2 duration-200">
                        {filteredSearchBlocks.map(b => (
                          <div 
                            key={b.id} 
                            className="px-4 py-3 hover:bg-[#faf9f6] cursor-pointer border-b border-[#f5f5f4] last:border-0 transition-colors"
                    ...
                    {showResults && searchQuery && filteredSearchBlocks.length === 0 && !selectedBlockId && (
                      <div className="absolute z-50 w-full mt-1 bg-white border border-[#d6d3d1] rounded-xl p-4 text-center text-[10px] text-[#a8a29e] italic shadow-xl">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        ...
        <div className="flex items-center gap-3 w-full sm:w-auto">
          <button 
            ...
            className="flex-1 sm:flex-none bg-white border border-[#d6d3d1] hover:bg-stone-50 text-[#57534e] px-4 py-2.5 rounded-lg flex items-center justify-center gap-2 font-bold text-xs shadow-sm transition-all"
          >
            <i className="fas fa-file-excel text-green-600"></i> Detailed Export
          </button>
          
          <div className="flex-1 sm:flex-none bg-white border border-[#d6d3d1] px-4 sm:px-6 py-2.5 sm:py-3 rounded-lg flex items-center justify-center sm:justify-start space-x-3 sm:space-x-4 shadow-sm">
            <div className="text-[#a8a29e]">
              <i className="fas fa-bolt text-lg"></i>
            </div>
            <div className="text-right">
              <div className="text-[10px] font-medium text-[#78716c] leading-none">Active</div>
              <div className="text-lg sm:text-xl font-semibold text-[#292524]">
