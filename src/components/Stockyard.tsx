...
export const Stockyard: React.FC<Props> = ({ blocks, onRefresh, activeStaff, isGuest }) => {
  ...
  const commonSelectStyle = "w-full bg-white border border-[#d6d3d1] rounded-lg py-2 px-3 text-xs font-medium focus:border-[#5c4033] outline-none shadow-sm transition-all";
          ...
          {yardBlocks.map(block => {
            ...
            return (
              <div key={block.id} className={`bg-white border border-[#d6d3d1] rounded-2xl p-4 shadow-sm transition-all ${isSelected ? 'ring-2 ring-[#5c4033] bg-[#fffaf5]' : ''}`}>
                ...
                <div className="flex justify-between items-center mt-4 pt-3 border-t border-[#f5f5f4]">
                   <div className="bg-white border border-[#d6d3d1] px-3 py-1 rounded-lg text-[10px] font-black text-[#5c4033] shadow-sm">
                      {block.msp || 'MSP'}
                   </div>
                   <div className="flex gap-2">
                      {!isGuest && (
                        <>
                           <button onClick={() => { setEditingBlock(block); setEditFormData(block); }} className="w-9 h-9 flex items-center justify-center bg-white border border-[#d6d3d1] text-stone-500 rounded-xl hover:bg-stone-50 active:scale-95 shadow-sm"><i className="fas fa-pen text-xs"></i></button>
                           <button 
                             onClick={() => { 
                               setSaleFormData({ soldTo: '', billNo: '', soldSqFt: (block.totalSqFt || 0).toFixed(2) });
                               setSaleModalOpen({ open: true, block: block }); 
                             }}
                             className="w-9 h-9 flex items-center justify-center bg-amber-50 border border-amber-200 text-amber-600 rounded-xl active:scale-95 shadow-sm"
                           >
                             <i className="fas fa-shopping-cart text-xs"></i>
                           </button>
                           <button onClick={() => handleDelete(block.id, block.jobNo)} className="w-9 h-9 flex items-center justify-center bg-red-50 border border-red-100 text-red-400 rounded-xl hover:bg-red-50 active:scale-95 shadow-sm"><i className="fas fa-trash-alt text-xs"></i></button>
                        </>
                      )}
                   </div>
                </div>
