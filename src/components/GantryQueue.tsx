        ...
        {filtered.map(block => {
          ...
          return (
            <div key={block.id} className={`bg-white border border-[#d6d3d1] p-3 rounded-xl shadow-sm ${isSelected ? 'ring-2 ring-[#5c4033] bg-[#fffaf5]' : ''}`}>
              ...
              <div className="grid grid-cols-2 gap-2 my-2 pb-2 border-b border-[#f5f5f4]">
                  <button 
                    disabled={isDisabled}
                    onClick={() => db.updateBlock(block.id, { preCuttingProcess: block.preCuttingProcess === 'TENNAX' ? 'None' : 'TENNAX' }).then(onRefresh)} 
                    className={`py-1.5 rounded-lg text-[9px] font-bold border transition-all flex items-center justify-center gap-1.5 ${block.preCuttingProcess === 'TENNAX' ? 'bg-amber-100 border-amber-300 text-amber-900 shadow-inner' : 'bg-white border-stone-200 text-stone-400'} ${isDisabled ? 'opacity-50 cursor-not-allowed' : ''}`}
                  >
                    <i className={`fas fa-check-circle ${block.preCuttingProcess === 'TENNAX' ? 'text-amber-600' : 'text-stone-200'}`}></i> TNX
                  </button>
                  <button 
                    disabled={isDisabled}
                    onClick={() => db.updateBlock(block.id, { preCuttingProcess: block.preCuttingProcess === 'VACCUM' ? 'None' : 'VACCUM' }).then(onRefresh)} 
                    className={`py-1.5 rounded-lg text-[9px] font-bold border transition-all flex items-center justify-center gap-1.5 ${block.preCuttingProcess === 'VACCUM' ? 'bg-cyan-100 border-cyan-300 text-cyan-900 shadow-inner' : 'bg-white border-stone-200 text-stone-400'} ${isDisabled ? 'opacity-50 cursor-not-allowed' : ''}`}
                  >
                    <i className={`fas fa-check-circle ${block.preCuttingProcess === 'VACCUM' ? 'text-cyan-600' : 'text-stone-200'}`}></i> VAC
                  </button>
              </div>
              
              <div className="mt-2 flex justify-between items-center">
                 <div className="text-[9px] font-bold text-[#57534e] truncate max-w-[120px]">{block.material} {block.minesMarka ? `(${block.minesMarka})` : ''}</div>
                 
                 {!isGuest && canEdit && (
                   <div className="flex gap-1.5">
                      <button onClick={() => openSaleModal(block)} className="w-8 h-8 rounded-lg bg-amber-50 border border-amber-200 text-amber-600 flex items-center justify-center active:scale-95"><i className="fas fa-shopping-cart text-[9px]"></i></button>
                      <button onClick={() => { setEditingBlock(block); setEditFormData(block); }} className="w-8 h-8 rounded-lg bg-stone-50 border border-stone-200 text-stone-400 flex items-center justify-center active:scale-95"><i className="fas fa-pen text-[9px]"></i></button>
                      <button onClick={() => { 
                        if(!block.isToBeCut) { setQueueModal({ open: true, blockId: block.id }); } 
                        else { db.updateBlock(block.id, { isToBeCut: false }).then(onRefresh); }
                      }} className={`px-3 h-8 rounded-lg text-[9px] font-bold uppercase active:scale-95 transition-transform ${block.isToBeCut ? 'bg-orange-100 text-orange-700' : 'bg-[#5c4033] text-white'}`}>{block.isToBeCut ? 'Deque' : 'Queue'}</button>
                   </div>
                 )}
              </div>
            </div>
