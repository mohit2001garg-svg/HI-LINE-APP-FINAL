    ...
    <div className="space-y-6 pb-20">
      ...
      <div className="lg:hidden space-y-3">
        {readyBlocks.map(block => (
          <div key={block.id} className="bg-white border border-[#d6d3d1] rounded-xl p-3 shadow-sm animate-in fade-in">
            ...
            <div className="grid grid-cols-2 gap-x-2 gap-y-1 border-t border-[#f5f5f4] pt-1.5 mt-1.5 text-[8px] font-bold uppercase tracking-wider">
              ...
              <div className="flex justify-between">
                <span className="text-[#a8a29e]">Marka</span>
                <span className="text-[#5c4033] truncate ml-1">{block.minesMarka || '-'}</span>
              </div>
              <div className="flex justify-between">
            ...
            <div className="mt-3 pt-2 border-t border-[#f5f5f4]">
              {!isGuest && (
                <div className="flex flex-col gap-1.5">
                  {transferringId === block.id ? (
                      <div className="bg-[#f5f5f4] p-2 rounded-lg">
                        <div className="text-[7px] font-bold text-[#78716c] uppercase mb-1.5 text-center">Destination</div>
                        <div className="grid grid-cols-2 gap-1.5 animate-in fade-in">
                          {locations.map(loc => (
                            <button key={loc} onClick={() => handleTransfer(block.id, loc, block.company)} className="bg-white border border-[#d6d3d1] text-[#5c4033] px-1 py-1.5 rounded text-[8px] font-bold shadow-sm truncate active:scale-95">{loc}</button>
                          ))}
                          <button onClick={() => setTransferringId(null)} className="col-span-2 bg-stone-200 text-stone-600 py-1.5 rounded text-[8px] font-bold">Cancel</button>
                        </div>
                      </div>
                  ) : (
                    <button onClick={() => setTransferringId(block.id)} className="w-full bg-[#5c4033] text-white py-2 rounded-lg font-bold text-[10px] uppercase shadow-sm active:scale-95 transition-all">Move to Yard</button>
                  )}
                </div>
              )}
            </div>
        ...
        {readyBlocks.length === 0 && (
          <div className="py-8 text-center text-stone-400 italic text-[10px]">No ready stock available.</div>
        )}
      </div>
