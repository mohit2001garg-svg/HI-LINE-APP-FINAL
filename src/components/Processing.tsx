...
export const Processing: React.FC<Props> = ({ blocks, onRefresh, isGuest, activeStaff }) => {
  ...
  const [finishModalOpen, setFinishModalOpen] = useState<{ id: string; action: 'finish' | 'resin' | 'stockyard' } | null>(null);
  ...
  const openFinishModal = (block: Block, action: 'finish' | 'resin' | 'stockyard') => {
  ...
  const handleFinalizeBlock = async (e: React.FormEvent) => {
    ...
    try {
      ...
      } else if (finishModalOpen.action === 'resin') {
        await db.updateBlock(finishModalOpen.id, { ...updates, isSentToResin: true, processingStage: 'Resin Plant' });
      } else {
        await db.updateBlock(finishModalOpen.id, { ...updates, status: BlockStatus.IN_STOCKYARD, processingStage: 'Field', transferredToYardAt: new Date().toISOString() });
      }
      onRefresh(); setFinishModalOpen(null);
    } catch (err) { alert("Update failed."); } finally { setLoadingId(null); }
  };

  const commonInputStyle = "w-full bg-white border border-[#d6d3d1] rounded-lg px-2 py-1.5 text-xs font-medium focus:border-[#5c4033] outline-none shadow-sm transition-all";
    ...
    <div className="space-y-6 pb-24">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        ...
        <button 
          ...
          className="w-full sm:w-auto bg-white border border-[#d6d3d1] hover:bg-stone-50 text-[#57534e] px-4 py-2.5 rounded-lg flex items-center justify-center gap-2 font-bold text-xs shadow-sm transition-all"
        >
      ...
      <div className="space-y-3 lg:hidden">
        {processingBlocks.length > 0 ? (
              ...
              <div key={block.id} className={`bg-white border border-[#d6d3d1] p-3 rounded-xl shadow-sm ${isSelected ? 'ring-2 ring-[#5c4033] bg-[#fffaf5]' : ''}`}>
                <div className="flex justify-between items-start mb-1.5">
                  <div className="flex items-center gap-2">
                    {!isGuest && canEdit && (
                      <div onClick={() => handleToggleId(block.id)} className={`w-4 h-4 rounded border flex items-center justify-center ${isSelected ? 'bg-[#5c4033] border-[#5c4033]' : 'border-stone-300'}`}>
                  ...
                  <div className="text-right">
                    <div className="text-xs font-black text-[#5c4033]">{block.totalSqFt?.toFixed(2)} ft</div>
                    <span className={`inline-block text-[7px] px-1 py-0.5 rounded font-bold mt-0.5 ${Number(recovery) > 250 ? 'bg-emerald-50 text-emerald-700' : 'bg-stone-50 text-stone-600'}`}>
                      {recovery} ft/T
                    </span>
                  </div>
                </div>

                <div className="grid grid-cols-3 gap-2 text-[8px] border-t border-[#f5f5f4] pt-1.5 mt-1.5">
                  <div>
                    <span className="text-[#a8a29e] block font-bold">Material</span>
                    <span className="font-black text-[#44403c] truncate text-[9px]">{block.material}</span>
                  </div>
                  <div className="text-center">
                    <span className="text-[#a8a29e] block font-bold">Slabs</span>
                    <span className="font-black text-[#44403c] text-[9px]">{block.slabCount || '-'}</span>
                  </div>
                  <div className="text-right">
                    <span className="text-[#a8a29e] block font-bold">Size</span>
                    <span className="font-mono text-[#44403c] text-[9px]">{Math.round(block.slabLength || 0)} x {Math.round(block.slabWidth || 0)}</span>
                  </div>
                </div>
                ...
                <div className="flex flex-wrap justify-end gap-1 mt-2 pt-1.5 border-t border-[#f5f5f4]">
                  {!isGuest && canEdit ? (
                    <>
                      ...
                      <button onClick={() => openFinishModal(block, 'finish')} className="flex-1 px-1.5 py-1 bg-[#5c4033] text-white rounded-lg text-[8px] font-bold uppercase tracking-wider shadow-sm">Ready</button>
                      <button onClick={() => handleDelete(block.id, block.jobNo)} className="px-1.5 py-1 text-red-400 bg-red-50 rounded-lg"><i className="fas fa-trash-alt text-[9px]"></i></button>
                    </>
                  ) : (
                    <span className="text-[8px] text-[#a8a29e] italic font-bold w-full text-center py-1 bg-[#f5f5f4] rounded">Read Only</span>
                  )}
                </div>
              </div>
            );
          ...
          <div className="py-8 text-center text-stone-400 italic text-[10px]">No active processing blocks</div>
        )}
      </div>
          ...
          <div className="bg-white rounded-2xl w-full max-w-sm p-6 shadow-2xl animate-in zoom-in-95">
            ...
            <form onSubmit={handleFinalizeBlock} className="space-y-4">
              ...
              <div className="flex gap-3 pt-4">
                ...
                <button type="submit" disabled={!!loadingId} className={`flex-[2] text-white py-3 rounded-xl font-bold text-[10px] uppercase tracking-widest shadow-xl active:scale-95 transition-all ${finishModalOpen.action === 'finish' ? 'bg-[#5c4033] hover:bg-[#4a3b32]' : finishModalOpen.action === 'resin' ? 'bg-cyan-600 hover:bg-cyan-700' : 'bg-emerald-600 hover:bg-emerald-700'}`}>
                  {loadingId ? <i className="fas fa-spinner fa-spin"></i> : (finishModalOpen.action === 'finish' ? 'Finalize' : finishModalOpen.action === 'resin' ? 'To Resin' : 'To Yard')}
                </button>
              </div>
            </form>
          </div>
