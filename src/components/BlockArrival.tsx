...
export const BlockArrival: React.FC<Props> = ({ onSuccess, activeStaff, blocks }) => {
  ...
  return (
    <div className="flex flex-col lg:flex-row gap-6 pb-32">
      <div className="flex-1 bg-white border-2 border-[#5c4033] rounded-2xl p-4 lg:p-8 shadow-2xl relative overflow-hidden">
        <div className="flex justify-between items-center mb-6 border-b border-stone-100 pb-3">
          <h2 className="text-xl lg:text-2xl font-black text-[#5c4033] uppercase italic">New Arrival</h2>
          {!isGuest && (
            <div className="flex gap-2">
               <input type="file" ref={fileInputRef} className="hidden" accept=".xlsx" onChange={handleImportExcel} />
               <button onClick={() => fileInputRef.current?.click()} className="bg-[#faf9f6] hover:bg-[#f5f5f4] text-[#57534e] px-3 py-1.5 rounded-lg text-[10px] font-bold border border-[#d6d3d1] transition-all flex items-center gap-1.5">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="grid grid-cols-3 gap-2 bg-[#faf9f6] p-3 rounded-xl border border-[#d6d3d1]">
            <div><label className={labelStyle}>Len (In)</label><input type="number" step="0.01" required className={commonInputStyle} placeholder="0.00" value={formData.length} onChange={e => setFormData({...formData, length: e.target.value})} disabled={isSubmitting || isGuest} /></div>
            <div><label className={labelStyle}>Hgt (In)</label><input type="number" step="0.01" required className={commonInputStyle} placeholder="0.00" value={formData.height} onChange={e => setFormData({...formData, height: e.target.value})} disabled={isSubmitting || isGuest} /></div>
            <div><label className={labelStyle}>Wid (In)</label><input type="number" step="0.01" required className={commonInputStyle} placeholder="0.00" value={formData.width} onChange={e => setFormData({...formData, width: e.target.value})} disabled={isSubmitting || isGuest} /></div>
          </div>
          ...
          <button type="submit" disabled={isSubmitting || isGuest} className="w-full bg-[#5c4033] hover:bg-[#4a3b32] disabled:bg-[#d6d3d1] text-white font-black uppercase tracking-widest py-4 rounded-xl shadow-lg transform active:scale-[0.98] transition-all mt-2 text-xs">
      ...
      <div className="lg:w-80 space-y-4">
        <div className="bg-emerald-50 border border-emerald-100 p-4 rounded-xl">
          <div className="text-[10px] font-bold text-emerald-800 uppercase tracking-wider mb-0.5">Today's Arrivals</div>
          <div className="text-2xl font-black text-emerald-900">{blocks.filter(b => b.status === BlockStatus.GANTRY && b.arrivalDate === new Date().toISOString().split('T')[0]).length}</div>
        </div>
        
        <div className="bg-white border border-[#d6d3d1] rounded-xl overflow-hidden shadow-sm flex-1">
          <div className="bg-[#f5f5f4] px-4 py-2 border-b border-[#d6d3d1]">
            <h3 className="text-[10px] font-bold text-[#78716c] uppercase">Recent Log (Factory)</h3>
          </div>
