...
export const Dashboard: React.FC<Props> = ({ blocks, onViewChange }) => {
  ...
  const StatCard = ({ icon, label, value, subValue, highlight, onClick }: any) => (
    <button 
      onClick={onClick}
      className={`group bg-white border border-[#d6d3d1] p-4 lg:p-6 rounded-xl text-left transition-all duration-300 hover:shadow-lg hover:border-[#a8a29e] relative overflow-hidden`}
    >
      <div className="flex justify-between items-start mb-3 lg:mb-4">
         <div className={`w-8 h-8 lg:w-10 lg:h-10 flex items-center justify-center rounded-lg ${highlight ? 'bg-[#5c4033] text-white' : 'bg-[#faf9f6] text-[#78716c]'} group-hover:bg-[#5c4033] group-hover:text-white transition-colors`}>
           <i className={`fas ${icon} text-sm lg:text-lg`}></i>
         </div>
         {highlight && (
           <span className="inline-flex items-center gap-1 px-1.5 py-0.5 rounded bg-amber-50 border border-amber-100">
             <span className="w-1 h-1 rounded-full bg-amber-600"></span>
             <span className="text-[8px] lg:text-[10px] font-bold text-amber-700 uppercase">Active</span>
           </span>
         )}
      </div>
      
      <div>
        <div className="text-[10px] lg:text-xs font-bold text-[#78716c] mb-0.5 lg:mb-1 uppercase tracking-wider">{label}</div>
        <div className="text-xl lg:text-3xl font-black text-[#292524] tracking-tight">
          {value}
        </div>
        <div className="mt-2 lg:mt-3 pt-2 lg:pt-3 border-t border-[#e7e5e4] flex items-center gap-2">
          <div className="text-[9px] lg:text-xs font-medium text-[#a8a29e] truncate">
                  ...
                  <div key={b.id} className="bg-white border border-[#d6d3d1] p-4 rounded-xl shadow-sm hover:shadow-md transition-all">
                     <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4">
                        ...
                        <div className="flex items-start gap-3">
                           <div className={`w-10 h-10 rounded-lg flex items-center justify-center text-lg font-bold border shrink-0 ${
                           ...
                           <div className="min-w-0">
                              <div className="flex items-center gap-2">
                                 <h4 className="text-lg font-black text-[#292524] tracking-tight">#{b.jobNo}</h4>
                                 <span className="px-1.5 py-0.5 bg-[#f5f5f4] text-[#78716c] text-[8px] font-bold uppercase rounded border border-[#e7e5e4]">{b.status}</span>
                              </div>
                              <div className="text-xs font-bold text-[#5c4033] mt-0.5 uppercase truncate">{b.company}</div>
                              <div className="text-[10px] text-[#a8a29e] mt-0.5 font-bold uppercase truncate">{b.material} {b.minesMarka ? `(${b.minesMarka})` : ''}</div>
                           </div>
                        </div>

                        {/* Details Grid */}
                        <div className="flex-1 grid grid-cols-2 lg:grid-cols-4 gap-2 text-[10px]">
                           <div className="bg-[#faf9f6] p-2 rounded-lg border border-[#e7e5e4]">
                              <div className="text-[8px] font-bold text-[#a8a29e] uppercase mb-0.5">Weight / Dims</div>
                              <div className="font-bold text-[#292524]">{b.weight?.toFixed(2)} T</div>
                              <div className="text-[9px] text-[#78716c] font-mono mt-0.5">{Math.round(b.length)}x{Math.round(b.height)}x{Math.round(b.width)}</div>
                           </div>
                           
                           <div className="bg-[#faf9f6] p-2 rounded-lg border border-[#e7e5e4]">
                              <div className="text-[8px] font-bold text-[#a8a29e] uppercase mb-0.5">Location / Status</div>
                              <div className="font-bold text-[#292524] truncate">
                           ...
                           <div className="bg-[#faf9f6] p-2 rounded-lg border border-[#e7e5e4]">
                              <div className="text-[8px] font-bold text-[#a8a29e] uppercase mb-0.5">Output / Area</div>
                              <div className="font-bold text-[#292524]">{b.totalSqFt ? `${b.totalSqFt.toFixed(2)} sqft` : '-'}</div>
                              <div className="text-[9px] text-[#78716c] mt-0.5">{b.slabCount ? `${b.slabCount} slabs` : '-'}</div>
                           </div>

                           <div className="bg-[#faf9f6] p-2 rounded-lg border border-[#e7e5e4]">
                              <div className="text-[8px] font-bold text-[#a8a29e] uppercase mb-0.5">Timestamps</div>
                              <div className="font-bold text-[#292524]">
                                 In: {new Date(b.arrivalDate).toLocaleDateString()}
                              </div>
                              {b.soldAt && (
                                 <div className="text-[9px] text-green-600 font-bold mt-0.5 uppercase">Sold: {new Date(b.soldAt).toLocaleDateString()}</div>
                              )}
                           </div>
                        </div>
                     </div>
