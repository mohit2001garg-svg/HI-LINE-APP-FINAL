      ...
      <div className="bg-white border border-[#d6d3d1] rounded-xl overflow-hidden shadow-sm">
        ...
        <div className="lg:hidden divide-y divide-stone-100">
          {soldBlocks.map(block => {
            ...
            return (
              <div key={block.id} className="p-4 bg-white active:bg-stone-50 transition-colors">
                <div className="flex justify-between items-start mb-2">
                  <div>
                    <div className="text-[10px] font-bold text-[#a8a29e] uppercase mb-0.5">{new Date(block.soldAt!).toLocaleDateString()}</div>
                    <div className="text-sm font-black text-[#292524] uppercase">{block.soldTo}</div>
                    <div className="text-[10px] font-bold text-[#78716c] uppercase">Bill: {block.billNo}</div>
                  </div>
                  <div className="text-right">
                    {isAreaSale ? (
                        <div className="text-base font-black text-[#5c4033]">{block.totalSqFt?.toFixed(2)} ft</div>
                    ) : (
                        <div className="text-base font-black text-[#5c4033]">{block.weight?.toFixed(2)} T</div>
                    )}
                  </div>
                </div>
                <div className="flex justify-between items-center">
                  ...
                  {!isGuest && (
                    <div className="flex gap-2">
                      <button onClick={() => handleEditClick(block)} className="w-8 h-8 rounded-lg bg-stone-50 border border-stone-200 text-stone-400 flex items-center justify-center"><i className="fas fa-pen text-[10px]"></i></button>
                      <button onClick={() => handleDelete(block.id, block.company)} className="w-8 h-8 rounded-lg bg-red-50 border border-red-100 text-red-400 flex items-center justify-center"><i className="fas fa-trash-alt text-[10px]"></i></button>
                    </div>
                  )}
                </div>
              </div>
          ...
          {soldBlocks.length === 0 && (
            <div className="p-12 text-center text-[#a8a29e] italic text-xs">No sales found.</div>
          )}
        </div>
